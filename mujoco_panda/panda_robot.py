...
MODEL_PATH = os.environ['HOME'] + \
    'junyi/3D_arm_whipping/mujoco_panda/models/self_franka_panda.xml'# franka_panda.xml -> self_franka_panda.xml
...
class PandaArm(MujocoRobot):
    ...
    def __init__(self, model_path=MODEL_PATH, render=True, config=DEFAULT_CONFIG, compensate_gravity=True, grav_comp_model_path=None, smooth_ft_sensor=False, **kwargs):
        ...
        self.D=1.2
        self.step_time=0.01
        self.pos_ini=[0.2, -0.785, 0.1, -2.356, 0, 1.571, 0.785]
        self.pos_end=[-0.2, 0.785, -0.1, 2.356, 0.2, 1.571, 0.785]
        MjSim = cymj.MjSim
        self.mj_model=mjp.load_model_from_path('/home/jy/junyi/3D_arm_whipping/mujoco_panda/mujoco_panda/models/self_franka_panda.xml')
        self.mj_sim=mjp.MjSim(self.mj_model)
    @property
    ...
    def hard_set_joint_velocities(self, values, indices=None):
        """
        ...
        self._sim.data.qvel[indices] = values

    def min_jerk(self):
        pos_ini=self.pos_ini
        pos_end=self.pos_end
        pos_tmp=[0 for _ in range (len(pos_ini))]
        vel_tmp=[0 for _ in range (len(pos_ini))]
        for i in range(len(pos_ini)):
            pos_tmp[i]=pos_ini[i]+(pos_end[i]-pos_ini[i])*((10* (self.step_time/self.D)**3)-15* (self.step_time/self.D)**4+ 6*(self.step_time/self.D)**5)
            vel_tmp[i]=1.0/self.D *(pos_end[i]- pos_ini[i])* (30*(self.step_time/self.D)**2-60*(self.step_time/self.D)**3 + 30*(self.step_time/self.D)**4)
        return pos_tmp, vel_tmp

    def reset(self):
        self.mj_sim.reset()
        self._sim.data.qpos[:] = 0
    # def whip_pose(self):
    #     """
    #     whip final pose:[]
    #     """
    #     self.hard_set_joint_positions(pos)
    ...
    def fullRobotWithTorqueActuators(cls, **kwargs):
        ...
        """
        model_path = os.environ['HOME'] + \
            'junyi/3D_arm_whipping/mujoco_panda/models/self_franka_panda.xml'
    ...
    def fullRobotWithPositionActuators(cls, **kwargs):
        ...
        """
        model_path = os.environ['HOME'] + \
            'junyi/3D_arm_whipping/mujoco_panda/models/franka_panda_pos.xml'
        return cls(model_path=model_path, **kwargs)
    ...
    def withTorqueActuators(cls, **kwargs):
        ...
        """
        model_path = os.environ['HOME'] + \
            'junyi/3D_arm_whipping/mujoco_panda/models/franka_panda_no_gripper.xml'
        return cls(model_path=model_path, **kwargs)
    ...
    def withPositionActuators(cls, **kwargs):
        ...
        """
        model_path = os.environ['HOME'] + \
            'junyi/3D_arm_whipping/mujoco_panda/models/franka_panda_pos_no_gripper.xml'
        return cls(model_path=model_path, **kwargs)
